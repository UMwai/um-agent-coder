#!/usr/bin/env python3
"""Submit the AI Financial Advisor Engine design task to the live Gemini Intelligence Layer."""
...
async def main():
    async with httpx.AsyncClient() as client:
        print("=" * 70)
        print("Submitting AI Financial Advisor Engine task")
        ...
        # Save full response
        out_path = "/home/umwai/um-agent-coder/scripts/gemini_umcfo_response.md"
        with open(out_path, "w") as f:
            f.write("# AI Financial Advisor Engine - Gemini Pro 3.1 Response\n\n")
            f.write(f"**Model**: {data['model']}  \n")
            f.write(f"**Duration**: {data['duration_ms']}ms  \n")
            f.write(f"**Tokens**: {usage.get('total_tokens', 0):,}  \n")
            if data.get("evaluation"):
                f.write(f"**Eval Score**: {data['evaluation']['score']:.2f}  \n")
            f.write(f"\n---\n\n")
