...
from .data_modes import DataAgentMode, DataModeConfig, DataModeManager
...
class DataAgent:
    ...
    def _analyze_data_task(self, prompt: str, mode_config: DataModeConfig) -> TaskAnalysis:
    ...
    def _execute_data_pipeline(
        self, plan: ExecutionPlan, mode_config: DataModeConfig
    ) -> list[dict[str, Any]]:
...
"""
        ...
        return type("Result", (), {"success": True, "data": response})()
...
"""
    ...
    def _extract_insights(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        insights = {"data_quality_issues": [], "statistics": {}, "recommendations": []}
    ...
    def _calculate_data_volume(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        volume = {"rows_processed": 0, "tables_analyzed": 0, "pipelines_created": 0}
        ...
        for result in results:
            if result.get("tool") in ["SQLExecutor", "DataProfiler"]:
                lineage["data_sources"].append(
                    {"tool": result["tool"], "description": result["description"]}
                )
                ...
                lineage["outputs"].append(
                    {
                        ...
                        "artifacts": (
                            list(result.get("data", {}).keys()) if result.get("data") else []
                        ),
                    }
                )
