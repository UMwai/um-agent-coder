...
"""
    ...
    def _get_approval(self, task_analysis: TaskAnalysis, plan: ExecutionPlan) -> bool:
        ...
        complexity = task_analysis.complexity
        if complexity >= 8:
            comp_style = ANSI.FAIL
        elif complexity >= 5:
            comp_style = ANSI.WARNING
        else:
            comp_style = ANSI.GREEN

        print(f"{ANSI.style('Type:', ANSI.BOLD)} {task_analysis.task_type.value}")
        print(
            f"{ANSI.style('Complexity:', ANSI.BOLD)} {ANSI.style(f'{complexity}/10', comp_style)}"
        )
        print(
            f"{ANSI.style('Estimated tokens:', ANSI.BOLD)} {task_analysis.estimated_tokens:,}"
        )
        print(
            f"{ANSI.style('Estimated cost:', ANSI.BOLD)} {ANSI.style(f'${plan.estimated_cost:.4f}', ANSI.GREEN)}"
        )
        print(
            f"{ANSI.style('Estimated time:', ANSI.BOLD)} {ANSI.style(f'{plan.estimated_time_minutes:.1f} min', ANSI.CYAN)}"
        )

        if task_analysis.potential_risks:
            print(f"\n{ANSI.style('⚠️  POTENTIAL RISKS:', ANSI.FAIL)}")
            for risk in task_analysis.potential_risks:
                print(f"  {ANSI.style('•', ANSI.FAIL)} {risk}")

        print(
            f"\n{ANSI.style(f'EXECUTION PLAN ({len(plan.steps)} steps)', ANSI.BLUE + ANSI.BOLD)}"
        )
        print("-" * 60)
        for i, step in enumerate(plan.steps, 1):
            print(f"  {ANSI.style(str(i) + '.', ANSI.CYAN)} {step.description}")
