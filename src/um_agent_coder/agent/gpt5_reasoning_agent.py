...
class GPT5ReasoningAgent(BaseAgent):
    ...
    def _chain_of_thought_reasoning(
        ...
        final_answer_prompt = (
            f"Based on the reasoning above, provide a concise final answer to: {prompt}"
        )
    ...
    def _reflexion_reasoning(
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a reasoning agent that critically evaluates and improves your own thinking.",
            },
            {
                "role": "user",
                "content": f"Problem: {prompt}\n\nContext: {json.dumps(context) if context else 'None'}",
    ...
    def _standard_reasoning(
        ...
        messages = [
            {
                "role": "system",
                "content": "You are GPT-5, an advanced AI assistant with superior reasoning capabilities.",
            },
            {
                "role": "user",
                "content": f"Problem: {prompt}\n\nContext: {json.dumps(context) if context else 'None'}",
    ...
    def _extract_confidence(self, text: str) -> float:
        ...
        patterns = [r"confidence[:\s]+(\d+)[/\s]", r"(\d+)\s*(?:out of|/)\s*10", r"rate[:\s]+(\d+)"]
    ...
    def get_reasoning_summary(self) -> str:
        ...
        for step in self.reasoning_history:
            ...
            summary += (
                ...
                else f"  Thought: {step.thought}\n"
            )
            if step.confidence:
                summary += f"  Confidence: {step.confidence:.2f}\n"
            summary += "\n"

        avg_confidence = sum(s.confidence for s in self.reasoning_history) / len(
