...
from .modes import AgentMode, ModeConfig, ModeManager
...
class RooAgent:
    ...
    def run(self, prompt: str, mode: Optional[AgentMode] = None) -> dict[str, Any]:
        ...
        try:
            ...
            if self.require_approval and not self._should_auto_approve(execution_plan, mode_config):
                approval = self._get_interactive_approval(
                    task_analysis, execution_plan, mode_config
                )
                if not approval:
                    return {
            ...
            response = self._generate_mode_aware_response(enhanced_prompt, results, mode_config)
            ...
            return {
                ...
                "task_id": task_id,
                "mode": mode.value,
                "mode_config": {"name": mode_config.name, "description": mode_config.description},
                "task_analysis": {
                ...
                "execution": {
                    ...
                    "tools_used": list({r.get("tool", "") for r in results if r.get("tool")}),
                },
    ...
    def _create_mode_aware_plan(
        self, analysis: TaskAnalysis, prompt: str, mode_config: ModeConfig
    ) -> ExecutionPlan:
    ...
    def _execute_with_mode(
        self, plan: ExecutionPlan, mode_config: ModeConfig
    ) -> list[dict[str, Any]]:
        ...
        for i, step in enumerate(plan.steps):
            ...
            self.cost_tracker.track_step(
                tokens=step.estimated_tokens, cost=self._calculate_step_cost(step.estimated_tokens)
            )
    ...
    def _generate_mode_aware_response(
        self, prompt: str, results: list[dict[str, Any]], mode_config: ModeConfig
    ) -> str:
...
Response:"""
    ...
    def _format_results(self, results: list[dict[str, Any]]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            formatted.append(f"{i}. [{status}] {result['description']}")
            if not result["success"] and result.get("error"):
                formatted.append(f"   Error: {result['error']}")
        return "\n".join(formatted)

    def _get_interactive_approval(
        self, analysis: TaskAnalysis, plan: ExecutionPlan, mode_config: ModeConfig
    ) -> bool:
        ...
        if response == "m":
            ...
            try:
                ...
                return self._get_interactive_approval(
                    analysis, plan, self.mode_manager.get_current_mode()
                )
            except Exception:
