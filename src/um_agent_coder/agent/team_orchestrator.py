...
class CostOptimizer:
    ...
    def select_optimal_model(
        self, task: AgentTask, available_budget: float, context: TeamContext
    ) -> str:
        ...
        if not suitable_models:
            ...
            return min(MODEL_CONFIGS.items(), key=lambda x: x[1].input_cost_per_1k)[0]

        # Calculate cost-effectiveness score
        scores = []
        for name, config in suitable_models:
            estimated_cost = config.estimate_cost(
                task.estimated_tokens, task.estimated_tokens // 2  # Rough output estimate
            )
        ...
        if not scores:
            ...
            return min(suitable_models, key=lambda x: x[1].estimate_cost(100, 50))[0]
    ...
    def update_cache(self, task_hash: str, result: Any):
        ...
        self.cache[task_hash] = {"result": result, "timestamp": datetime.now().isoformat()}
    ...
    def update_model_performance(self, model: str, success: bool, latency: float):
        ...
        if model not in self.model_performance:
            self.model_performance[model] = {"success_count": 0, "total_count": 0, "avg_latency": 0}
        ...
        perf["avg_latency"] = (perf["avg_latency"] * (perf["total_count"] - 1) + latency) / perf[
            "total_count"
        ]
...
class TeamOrchestrator:
    ...
    async def execute_team_task(
        ...
        context = TeamContext(
            objective=objective, total_budget=budget, constraints=constraints or {}
        )

        # Detect if specialized agents are needed
        if use_specialized_agents:
            specialized_agent = SpecializedAgentFactory.recommend_agent(
    ...
    async def _planning_phase(self, context: TeamContext) -> dict[str, Any]:
        ...
        model = self.optimizer.select_optimal_model(
            planning_task, context.total_budget - context.spent_budget, context
        )
    ...
    def _decompose_into_tasks(self, plan: dict[str, Any], context: TeamContext) -> list[AgentTask]:
    ...
    async def _execute_tasks(self, tasks: list[AgentTask], context: TeamContext) -> dict[str, Any]:
    ...
    async def _execute_single_task(self, task: AgentTask, context: TeamContext) -> Any:
        ...
        try:
            ...
            if task.assigned_model:
                ...
                task.actual_cost = config.estimate_cost(
                    task.estimated_tokens, task.estimated_tokens // 2
                )
            ...
            self.optimizer.update_model_performance(task.assigned_model, True, latency)
            ...
            # Update performance metrics
            if task.assigned_model:
                self.optimizer.update_model_performance(task.assigned_model, False, 0)

            raise

    async def _integration_phase(self, results: dict[str, Any], context: TeamContext) -> Any:
        ...
        available_budget = context.total_budget - context.spent_budget
        model = self.optimizer.select_optimal_model(integration_task, available_budget, context)
    ...
    async def _execute_specialized_task(
        self, agent, objective: str, context: TeamContext
    ) -> dict[str, Any]:
        ...
        agent.prepare_prompt(objective, context.constraints)
        ...
        if agent.capabilities.preferred_models:
            ...
            model = self.optimizer.select_optimal_model(
                task, context.total_budget - context.spent_budget, context
            )
    ...
    def _get_agents_summary(self) -> dict[str, Any]:
        ...
        summary = {"general_agents": [], "specialized_agents": []}
    ...
    def _get_performance_metrics(self) -> dict[str, Any]:
        ...
        # Calculate metrics
        avg_cost_per_task = (
            sum(task.actual_cost for task in self.completed_tasks.values()) / total_tasks
        )
...
async def main():
    ...
    result = await orchestrator.execute_team_task(
        ...
        budget=5.0,  # $5 budget
        constraints={"language": "python", "framework": "fastapi", "include_tests": True},
    )
