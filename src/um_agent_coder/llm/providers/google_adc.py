...
class GoogleADCProvider(LLM):
    ...
    def __init__(self, model: str = "gemini-1.5-pro-latest", 
        ...
        try:
            ...
            print("Warning: 'google-cloud-aiplatform' not found. ADC might not work as expected without it.")
    ...
    def chat(self, prompt: str, messages: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        try:
            if self.use_vertex:
                ...
                # Standard AI Studio
                chat = self.client.start_chat(history=history)
                response = chat.send_message(prompt, generation_config=genai.types.GenerationConfig(**config))
                return response.text
