...
Available Modes:
  ...
  - data_analyst: Analysis, reporting, visualization
    ...
    parser.add_argument("--data", "-d", help="Data source (file path or connection string)")
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--model", help="Specific LLM model to use (e.g., claude-3.5-sonnet, gpt-4o)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-profile", action="store_true", help="Disable automatic data profiling"
    )
    parser.add_argument(
        "--no-validate", action="store_true", help="Disable automatic data validation"
    )
    parser.add_argument("--export-lineage", help="Export data lineage to file after execution")
    ...
    try:
        ...
        result = agent.run(
            prompt=args.prompt, mode=mode, data_source=args.data, output_format=args.output
        )
