        ...
        try:
            ...
            if validate:
                ...
                if not validation["valid"]:
                    return ToolResult(
                        success=False, error=f"Validation failed: {validation['error']}"
                    )
...
class SmartCodeSearcher(Tool):
    ...
    def _regex_search(
        self, pattern: str, directory: str, file_types: Optional[list[str]], context_lines: int
    ) -> list[dict[str, Any]]:
    ...
    def _search_file(
        self, file_path: Path, pattern: str, context_lines: int
    ) -> list[dict[str, Any]]:
    ...
    def _semantic_search(
        self, query: str, directory: str, file_types: Optional[list[str]]
    ) -> list[dict[str, Any]]:
    ...
    def _ast_search(
        self, pattern: str, directory: str, file_types: Optional[list[str]]
    ) -> list[dict[str, Any]]:
        ...
        **kwargs,
        ...
        try:
            ...
            if safe_mode and not self._is_safe_command(command):
                return ToolResult(success=False, error=f"Command blocked by safe mode: {command}")
...
class SmartProjectAnalyzer(Tool):
    ...
    def execute(self, directory: str = ".", **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, error=f"Analysis error: {str(e)}")

    def _analyze_structure(self, path: Path) -> dict[str, Any]:
        """Analyze directory structure."""
        structure = {"directories": [], "files": [], "total_files": 0, "total_dirs": 0}
    ...
    def _calculate_metrics(self, path: Path) -> dict[str, Any]:
        ...
        metrics = {"total_lines": 0, "code_files": 0, "avg_file_size": 0}
