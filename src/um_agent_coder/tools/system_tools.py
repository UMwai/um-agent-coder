...
class CommandExecutor(Tool):
    ...
    def execute(
        ...
        if self.safe_mode and self._is_dangerous_command(command):
            return ToolResult(False, None, f"Command blocked for safety: {command}")
        ...
        try:
            ...
            try:
                ...
                return ToolResult(
                    success=success, data=result, error=stderr if not success else None
                )

            except subprocess.TimeoutExpired:
                process.kill()
                return ToolResult(False, None, f"Command timed out after {self.timeout} seconds")
